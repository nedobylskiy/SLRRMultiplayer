...
public class Valocity extends City {
    ...
    public Valocity() {
        ...
        map = new GroundRef(maps.city:0x00000001 r);
        nav = new Navigator(-23.482, -24.45, 5.828, maps.city.smallmap:0x00000001 r, maps.city.smallmap:0x00000002
        r, maps.city.smallmap:0x00000005 r, 8, 8, 8);
    ...
    public void enter(GameState prev_state) {
        if (prev_state instanceof RaceSetup) {
            ...
            if ((GameLogic.gameMode == GameLogic.GM_QUICKRACE)
                    || (GameLogic.gameMode == GameLogic.GM_DEMO)) {    //segitjuk az engine-t, ne kelljen a garazs kornyeket betolteni eloszor
//				posStart = map.getNearestCross( posStart );
            ...
            if (hour > 4 && hour < 22) {    //day time settings
                map.addTraffic(new GameRef(cars.traffic.Taxi:0x00000006 r),80, 2, 5, 2);
                map.addTraffic(new GameRef(cars.traffic.Ambulance:0x00000006 r),20, 2, 5, 2);
                map.addTraffic(new GameRef(cars.traffic.FireEngine:0x00000006 r),12, 2, 5, 2);
                map.addTraffic(new GameRef(cars.traffic.Coach:0x00000006 r),30, 2, 10, 2);
                map.addTraffic(new GameRef(cars.traffic.Schoolbus:0x00000006 r),55, 2, 10, 2);

                map.addTraffic(new GameRef(cars.traffic.ArmoredVan:0x00000006 r),20, 2, 5, 2);
                map.addTraffic(new GameRef(cars.traffic.Wagon:0x00000006 r),200, 2, 5, 2);
                map.addTraffic(new GameRef(cars.traffic.Erbilac:0x00000006 r),200, 2, 5, 2);
                map.addTraffic(new GameRef(cars.traffic.CivilVan:0x00000006 r),200, 2, 5, 2);
                ...
                } else if (GameLogic.gameMode == GameLogic.GM_DEMO) {
                    map.addTraffic(GRT_POLICECAR, 50, 2, 5, 2);
                }

                map.setPedestrianDensity(0.003);
            } else {    //night time settings

                map.addTraffic(new GameRef(cars.traffic.Erbilac:0x00000006 r),20, 2, 5, 2);
                map.addTraffic(new GameRef(cars.traffic.CivilVan:0x00000006 r),10, 2, 5, 2);
                map.addTraffic(new GameRef(cars.traffic.Taxi:0x00000006 r),150, 2, 5, 2);
                if (GameLogic.gameMode == GameLogic.GM_CARREER) {
            ...
            map.addPedestrianType(new GameRef(humans:0x0057 r));
            map.addPedestrianType(new GameRef(humans:0x0058 r));
            map.addPedestrianType(new GameRef(humans:0x0059 r));
            map.addPedestrianType(new GameRef(humans:0x005A r));
            map.addPedestrianType(new GameRef(humans:0x005B r));
            map.addPedestrianType(new GameRef(humans:0x0022 r));
        ...
        if (!(prev_state instanceof RaceSetup)) {
            ...
            } else if (GameLogic.gameMode == GameLogic.GM_DEMO) {//quick start
                osdEnabled = 0;
